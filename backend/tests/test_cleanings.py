...
class TestUpdateCleaning:
    ...
    async def test_update_cleaning_with_valid_input(
        ...
        cleaning_before_update = CleaningInDB(**response.json())
        ...
        assert updated_cleaning.id == cleaning_before_update.id  # make sure it's the same cleaning
        # make sure that any attribute we updated has changed to the correct value
        for i in range(len(attrs_to_change)):
            attr_to_change = getattr(updated_cleaning, attrs_to_change[i])
            assert attr_to_change != getattr(cleaning_before_update, attrs_to_change[i])
            assert attr_to_change == values[i]
        # make sure that no other attributes' values have changed
        for attr, value in updated_cleaning.dict().items():
            if attr not in attrs_to_change:
                assert getattr(cleaning_before_update, attr) == value
